...
def pick_levels(d):
    # Priority: PDH/PDL → Asia → London
    L = d.get("levels") or {}
    hi = L.get("pdh") or L.get("asia_high") or L.get("london_high")
    lo = L.get("pdl") or L.get("asia_low")  or L.get("london_low")
    return hi, lo
...
def main():
    ...
    if hi is None and lo is None:
        print("No PDH/PDL/Asia/London levels found.", file=sys.stderr); return
    ...
    for ts,o,h,l,c in src:
        if (not up) and (hi is not None) and (h >= hi):
            up=True
            post_discord(f"🟡 CANARY: crossed HIGH {hi:.2f} (c={c:.2f})")
            print(f"[CANARY] posted HIGH (c={c})", flush=True)
        if (not dn) and (lo is not None) and (l <= lo):
            dn=True
            post_discord(f"🟡 CANARY: crossed LOW {lo:.2f} (c={c:.2f})")
            print(f"[CANARY] posted LOW (c={c})", flush=True)
        if up and dn: break
