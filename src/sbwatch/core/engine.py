...
class SBEngine:
    ...
    def _update_window(self, bar: Bar) -> None:
        """Keep a rolling window of last N minutes for fallback refs."""
        self._window.append(bar)
        # our CSV is 1-minute bars, so limit by length for simplicity
    ...
    def on_bar(self, bar: Bar, pdh: Optional[float]=None, pdl: Optional[float]=None) -> Optional[TradeAlert]:
        # maintain window always
        self._update_window(bar)

        if not self._in_kz(bar.ts):
            # reset entry state outside kill-zone
            self.swept_side = None
            self.pending_fvg = None
            self.prev_bar = bar
            return None

        # choose reference levels: PDH/PDL if present, else rolling highs/lows
        ...
        # 1) Detect sweep of reference levels (if we have them)
        if ref_high is not None and swept_above(ref_high, bar.high, self.p.sweep_ticks):
            self.swept_side = "SHORT"
            self.basis = "ref-high sweep"
        if ref_low is not None and swept_below(ref_low, bar.low, self.p.sweep_ticks):
            self.swept_side = "LONG"
            self.basis = "ref-low sweep"

        # 2) After a sweep, wait for displacement
        if self.swept_side and is_displacement(bar, self.p.disp_min_ticks):
            if self.p.require_fvg:
                ...
                if self.prev_bar:
                    ...
                    if fvg:
                        self.pending_fvg = fvg  # (upper, lower, side)
            else:
                # 3b) No-FVG path: enter on displacement bar close with stop beyond range
                if self.swept_side == "SHORT":
                    entry = min(bar.open, bar.close)
                    stop = bar.high + self.p.stop_buf_ticks
                    risk = stop - entry
                    tp1 = entry - self.p.tp1_r * risk
                    tp2 = entry - self.p.tp2_r * risk
                else:
                ...
                alert = TradeAlert(
                    side=self.swept_side, entry=round(entry,2), stop=round(stop,2),
                    tp1=round(tp1,2), tp2=round(tp2,2),
                    r_multiple=round(r_mult,2), basis=self.basis or "SB sweep"
                )
                # reset after entry
                self.swept_side = None
                self.pending_fvg = None
        ...
        # 4) FVG refill path (only if required and we have one)
        if self.p.require_fvg and self.pending_fvg:
            ...
            if refill_hit(upper, lower, bar, side, self.p.refill_tol_ticks):
                ...
                r_mult = (abs(tp2 - entry) / risk) if risk > 0 else 0.0
                alert = TradeAlert(
                    side=side, entry=round(entry,2), stop=round(stop,2),
                    tp1=round(tp1,2), tp2=round(tp2,2),
                    r_multiple=round(r_mult,2), basis=self.basis or "SB sweep"
                )
                self.swept_side = None
